...
// ─────────────────────────────────────────────────────────────────────────────
// El suelo del arnés. Spec de origen: install-completion-floor, del repositorio original.
//
// Una instalación podía imprimir RSC_ONBOARDING_READY con tres markdown y un `.rsc.json`:
// `01-TOOLS/` no existía para el instalador.
//
...
test('floor: the installer creates the deterministic skeleton, and does it once', async () => {
  ...
  assert.ok(existsSync(join(cwd, 'docs/wiki/harness')), 'docs/wiki/harness/ también');
...
// Este fork no incluye constitución en el suelo: su flujo (grill-with-docs → to-spec → write-adr) no
// la escribe, así que exigirla dejaría a todo proyecto de software en INCOMPLETE para siempre.
test('floor: no plan requires a constitution — software and non-software pay the same floor', async () => {
  for (const record of [softwareRecord('complex'), softwareRecord('small'), opsRecord()]) {
    const cwd = freshWorkspace('same-floor');
    const plan = buildOnboardingPlan(record, scanProject(cwd));
    assert.deepEqual([...plan.floorPaths].sort(), ['01-TOOLS/_TEMPLATE/', 'docs/wiki/harness/']);
    ensureHarnessSkeleton(cwd);
    assert.deepEqual(missingHarnessFloor(cwd, plan), [], `${record.projectKind}: el esqueleto basta`);
  }
});
...
test('floor: readiness reports what is missing and an action that can create it', async () => {
  ...
  ensureHarnessSkeleton(cwd);
  assert.equal(harnessReadiness(cwd, plan).ready, true, 'con el esqueleto montado está listo');
  rmSync(join(cwd, '01-TOOLS'), { recursive: true });
  const report = harnessReadiness(cwd, plan);
  assert.equal(report.ready, false, 'sin la plantilla no está listo');
  assert.ok(report.missing.some((m) => m.includes('01-TOOLS/_TEMPLATE')));
...
// contrato que este ciclo añade a `llms.txt` — reparar el suelo y reejecutar para obtener el READY
// que ese documento declara obligatorio, sin que existiera comando que lo imprimiera.
// `scanProject` ya tenía el carve-out para `docs/wiki/harness/` y ninguno para `01-TOOLS/`.
...
// Mutante superviviente: quitar `docs/wiki/harness/` del suelo. Es una de las dos filas del suelo
...
test('floor: the minimum floor is both rows, and both are detected', () => {
  ...
  assert.ok(plan.floorPaths.includes('docs/wiki/harness/'));
  ensureHarnessSkeleton(cwd);
  rmSync(join(cwd, 'docs/wiki/harness'), { recursive: true });
  assert.ok(missingHarnessFloor(cwd, plan).some((m) => m.includes('docs/wiki/harness')));
});
...
// suelo NO cambie la identidad del plan, o el reintento moriría con RSC_PLAN_CHANGED.
test('floor: repairing the floor never changes the plan identity — the whole documented loop', async () => {
  ...
  ensureHarnessSkeleton(cwd);
  rmSync(join(cwd, '01-TOOLS'), { recursive: true });
  assert.ok(missingHarnessFloor(cwd, plan).some((m) => m.includes('01-TOOLS/_TEMPLATE')));

  // Lo que hace a continuación la acción nombrada: volver a montar el esqueleto.
  ensureHarnessSkeleton(cwd);
...
test('floor: a missing file is named as a file, without a trailing slash', () => {
  ...
  const missing = missingHarnessFloor(cwd, { floorPaths: ['docs/wiki/harness/user-profile.md'] });
  assert.deepEqual(missing, ['missing harness floor docs/wiki/harness/user-profile.md']);
});
