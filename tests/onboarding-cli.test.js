...
// ─────────────────────────────────────────────────────────────────────────────
// El suelo del arnés, desde el CLI. Spec de origen: install-completion-floor, del repositorio original.
//
// `rsc.js` imprimía RSC_ONBOARDING_READY justo después de aplicar, sin mirar si el arnés existía.
// Ahora el instalador monta el esqueleto y sólo afirma READY si el suelo existe de verdad.

const softwareComplete = [
  '--technical-level', 'mixed', '--accompaniment', 'L1', '--project-kind', 'software',
  '--software-scope', 'complex', '--goal', 'Build a substantial product', '--target', 'codex',
];
const onboardSoftware = (cwd) => {
  const preview = run(cwd, ['onboard', ...softwareComplete]);
  const id = preview.stdout.match(/Plan id: ([a-f0-9]{64})/)?.[1];
  return { id, applied: run(cwd, ['onboard', ...softwareComplete, '--accept-plan', id]) };
};

// Este fork no exige constitución: un proyecto de software con el esqueleto montado está listo.
test('floor: a software install whose skeleton is written is reported ready', () => {
  const cwd = fresh();
  const { applied } = onboardSoftware(cwd);
  assert.equal(applied.status, 0, applied.stderr);
  assert.match(applied.stdout, /RSC_ONBOARDING_READY/);
  assert.doesNotMatch(applied.stdout, /constitution/i, 'ningún rastro de la cadena retirada');
  assert.ok(existsSync(join(cwd, '01-TOOLS/_TEMPLATE/.env.example')));
  assert.ok(existsSync(join(cwd, 'docs/wiki/harness')));
});
...
test('floor: the agent handoff does not claim ready while the floor is incomplete', () => {
  ...
  assert.match(installed.stdout, /01-TOOLS\/_TEMPLATE|floor is incomplete/i, 'y tiene que decir por qué');
});
...
test('floor: a skeleton failure is not reported as an apply failure', () => {
  ...
  assert.ok(existsSync(join(cwd, 'docs/wiki/harness/user-profile.md')), 'lo aplicado sigue ahí');
});
