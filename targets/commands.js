...
const FIXED_COMMANDS = Object.freeze([
  ...
  skillCommand('update-docs', 'knowledge-ops'),
  {
    ...
    body: `Use the sello backing for this checkpoint: {{ARGS}}\nRun \`npx @damiangilgonzalez/harness sello freeze\`, complete the required review, then run \`sello approve\` only with the observed lenses. Do not treat freeze as approval.`,
  },
  ...
  skillCommand('security-scan', 'security-scan'),
  {
    ...
    body: `Invoke memory:learn for exactly one proposed lesson: {{ARGS}}\nAfter individual explicit approval, run \`npx @damiangilgonzalez/harness memory learn\` with its text, evidence, scope, confidence and \`--approve\`. Without that approval, write nothing.`,
  },
  {
    name: 'save-session', kind: 'memory', backing: 'memory:save',
    description: 'Force a local deterministic session checkpoint.',
    body: `Invoke memory:save with the current local session identifiers: {{ARGS}}\nRun \`npx @damiangilgonzalez/harness memory save --session <id>\`. Persist only allowed git and SDD ledger metadata; never include conversation or file content.`,
  },
  {
    ...
    body: `Invoke memory:resume for the current branch and worktree: {{ARGS}}\nRun \`npx @damiangilgonzalez/harness memory resume\`. Label a nearby branch result as nearby; never merge it silently into the exact continuation.`,
  },
]);
