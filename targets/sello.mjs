...
export const MESSAGES = {
  noReview: (tier) =>
    ...
    `Recover: run the \`review\` skill (it freezes, reviews and approves the sello), or \`npx @damiangilgonzalez/harness sello off\` to turn the sello off for this project.`,
  diverged: (paths) =>
    ...
    `Recover: re-run the \`review\` skill on the divergence (only what changed is re-reviewed), or \`npx @damiangilgonzalez/harness sello off\` for this project.`,
  baseMoved: () =>
    ...
    `Recover: re-run the \`review\` skill (only the divergence is re-reviewed), or \`npx @damiangilgonzalez/harness sello off\` for this project.`,
  corrupt: () =>
    `sello: the sello file is unreadable, which counts as "no review" — nothing is ever approved by default. ` +
    `Recover: re-run the \`review\` skill to produce a fresh sello, or \`npx @damiangilgonzalez/harness sello off\` for this project.`,
  blocked: (reason) =>
    `sello: the review left this change BLOCKED${reason ? ` (${reason})` : ''}. ` +
    `Recover: fix the blocking findings and re-run the \`review\` skill, or \`npx @damiangilgonzalez/harness sello off\` for this project.`,
  notFrozen: () =>
    `sello: nothing is frozen yet, so there is nothing to approve. ` +
    `Recover: run \`npx @damiangilgonzalez/harness sello freeze\` first, then review, then approve.`,
  staleFreeze: () =>
    `sello: the change mutated after it was frozen — approving now would seal bytes nobody reviewed. ` +
    `Recover: run \`npx @damiangilgonzalez/harness sello freeze\` again and re-review the divergence.`,
  ...
  badRiskConfig:
    'Recover: fix .rsc/sello-config.json — "raise" entries need a valid pattern under 200 chars; "lower" only accepts known classes and never "secrets" or "harness". `npx @damiangilgonzalez/harness sello off` always works, even with a broken config.',
  riskUnknown: () =>
