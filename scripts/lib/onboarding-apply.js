...
// instalador.
//
// Spec de origen: install-completion-floor, del repositorio original.
//
...
export const HARNESS_FLOOR_MINIMUM = [TEMPLATE_FLOOR, 'docs/wiki/harness/'];
...
export function ensureHarnessSkeleton(cwd = process.cwd()) {
  ...
  assertManagedPathsStayInsideRoot(cwd, ['01-TOOLS/', TEMPLATE_FLOOR, 'docs/wiki/harness/']);
  const created = [];
  for (const dir of ['docs/wiki/harness', '01-TOOLS/_TEMPLATE']) {
...
export function missingHarnessFloor(cwd = process.cwd(), plan = {}) {
  ...
  for (const path of declared) {
    ...
    // La barra sólo si el camino declarado la traía: una entrada que es un FICHERO no debe
    // mostrarse con barra, o invita a buscar un directorio que no existe.
...
export function harnessReadiness(cwd = process.cwd(), plan = {}) {
  ...
  return {
    ...
    action: missing.length
      ? 'invoke the `harness` skill to scaffold 01-TOOLS/ + docs/'
      : '',
  };
}
