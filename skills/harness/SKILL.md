...
1. **Detect → propose.** From the detected stacks/providers and the user's goals in `docs/wiki/harness/`, build a shortlist. Ask the CLI if unsure: `npx @damiangilgonzalez/harness consult "<stack + goal>"`. (Map e.g. detected Stripe→`stripe`, Postgres→`postgresdb`, Next→`nextjs`+`design`, a company/ops focus→`finance-ops`/`invoicing`/`gdpr-privacy`…)
