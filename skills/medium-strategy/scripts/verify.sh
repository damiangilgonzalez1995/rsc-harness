#!/usr/bin/env bash
#
# verify.sh — structural lint for a medium-strategy wiki under docs/wiki/medium/.
#
...
#        A record missing the metric/bet field is a HARD FAILURE — a decision
#        with no metric is an opinion, not a checkable bet.
#
#   Missing date or Decision line is also a hard failure. A missing/empty target
#   (a fresh account with no wiki yet) is reported and exits 0 — no false failure.
#   Pure bash + grep, no dependencies. It is a lint, not a strategy oracle.
#
# HOW TO RUN
#   ./verify.sh docs/wiki/medium/    # lint an account's strategy wiki
...
if [ ! -d "$TARGET" ]; then
  warn "directory not found: $TARGET — nothing to lint (fresh account?)"
  summary_exit
fi
