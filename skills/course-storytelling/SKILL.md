...
1. **Locate the profile.** Read the root `CLAUDE.md`, follow its `## Knowledge map` pointer to `docs/wiki/index.md`, and look for the entry into `docs/wiki/teaching/` (the `harness` Karpathy-wiki convention: compiled articles in `docs/wiki/teaching/`, raw user-pasted material in `docs/raw/teaching/`). No `CLAUDE.md`, no index entry, or a pointer that goes nowhere = ABSENT.
...
3. **If ABSENT or INCOMPLETE, STOP and interview** with the batched question script in `references/learner-grounding.md` — one focused batch at a time, wait, persist, continue. Then write the profile as wiki articles under `docs/wiki/teaching/` (`learner.md`, `audience.md`, `transformation.md`, `false-beliefs.md`, `constraints.md`, `index.md`), save pasted transcripts/outlines/slides verbatim under `docs/raw/teaching/` and link them from each article's `> Raw:` line, index the profile in `docs/wiki/index.md`, and ensure root `CLAUDE.md` carries the short pointer to that index (create it if absent; additive only). Article format and the exact `CLAUDE.md` snippet → `references/learner-grounding.md`.
4. **Only then proceed**, citing which articles you used ("grounded in `docs/wiki/teaching/learner.md` and `false-beliefs.md`") so every reframing is traceable to a real learner, not an imagined one.
...
1. **Ground.** Pass the gate above. Load learner, audience, transformation, false beliefs, constraints from `docs/wiki/teaching/` and cite them.
...
Beyond the gated learner profile, record the **course teaching conventions** at `docs/wiki/stack/course-storytelling.md` (or alongside the profile under `docs/wiki/teaching/`): the established narrative spine, the named mental models already coined, the Big Dominoes per module, and the teacher's Attractive Character. Recorded, not gated — update it as decisions are made, keep its entry current in `docs/wiki/index.md`, read it first on every use, and keep every reframing consistent with it. The wiki convention itself belongs to `../harness/SKILL.md`. If the project has no `docs` layer, skip this silently and proceed with the in-session profile.
