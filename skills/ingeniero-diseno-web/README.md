...
[Back to collection root](../../README.md)
...
> Frames are real artefacts rendered by the live React + Vite gallery at `demo/web-design-engineer-demo` (not included in this catalog) — same fonts, same palettes, same signature moves as the recipe spec. Each demo is in `src/recipes/<id>.tsx`.
...
Hash-routed URLs (`#/linear`, `#/aesop`, …) deep-link to any recipe. Press `H` to toggle the recipe HUD, `Esc` to return to the gallery. See the demo's own README (not included in this catalog) for layout details.
...
> The original Claude Design system prompt that inspired this skill is preserved at `dist/prompt/claude-design-system-prompt.md` (not included in this catalog).
