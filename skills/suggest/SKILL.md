---
name: suggest
description: "Always-on. Use whenever the current user turn would clearly benefit from an rsc skill that is not yet installed — detect the gap during normal agent use, name the skill, and (with a one-word confirm) install it via `npx @damiangilgonzalez/harness add <id>`. Triggers on capability intent in any language: building technology, creating content/assets, automating workflows, analyzing data, connecting tools, shipping/deploying, security, business ops, marketing, education, research, or company/documentation harness work."
...
1. `npx @damiangilgonzalez/harness catalog --available` lists every not-installed skill as
...
(`npx @damiangilgonzalez/harness list`).

`npx @damiangilgonzalez/harness consult "<task>"` is a **lexical** hint only: it keyword-matches, and returns
...
`npx @damiangilgonzalez/harness capabilities`: covered by a skill or agent → use it, say nothing.
