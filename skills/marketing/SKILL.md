...
5. **Write the microcopy and CTAs.** One primary CTA per viewport — choice kills conversion — with a value verb on the button ("Start free", not "Submit"), reassurance under it ("No credit card required"), and human empty/loading/error states. → `references/copy-frameworks.md`.
...
The headline is the highest-leverage copy on the page. Build it from the canvas, not from a thesaurus. It has one test: a stranger reads the hero and can say *what it is, who it's for, why it's better* — within 5 seconds. If it fails, rewrite it before anything else on the page.
...
- **Specificity** — replace an adjective with a number. "Fast" is a claim; "30 seconds" is evidence: "Cut review setup from 20 min to 30 sec".
...
Each section earns its place by doing one job. Cut any section with no job. This maps 1:1 to the `design` skill's section anatomy — `design` owns the visual treatment, this skill owns the words inside it. Within every section, climb feature → benefit → proof, stop at the rung the reader cares about (the benefit), and back it with proof.
...
When the task is bigger than the page — a launch, an email sequence, cross-channel messaging. One claim per asset: a post, an email, a section each carry one real claim, not five hedged ones.
...
Keep copy *SEO-aware* by default: one `<h1>`, scannable subheads that answer queries, benefit in the
first 160 chars. Keyword stuffing *lowers* AI citation. When the task is the visibility layer itself
— ranking on Google/Bing **and getting cited by AI engines** — go to `references/seo-geo.md`. It owns
technical + on-page SEO, JSON-LD schema, per-engine GEO, and keyword/SERP research.
...
- **GEO and this skill agree.** The top GEO moves (Princeton, KDD 2024) are *cite sources, add
  ...
  invented proof, one claim per asset" discipline this skill enforces everywhere else. Keyword
  stuffing lowers AI citation (~−10%). The proof rule is absolute: **never fabricate the stat or
  quote that earns the lift** — mark `[[NEEDS PROOF]]` and source it.
...
State once, enforce everywhere. These are defects, not style: they signal AI-template copy and erode trust. `scripts/verify.sh` greps for them.
...
| --- | --- |
| Skipping the brand study because "I know the brand" | Your default voice is AI-median. Run the gate; cite the articles. |
| Leaning on the image and letting the copy stay vague | Copy carries the value prop. Pass the 5s test in words. |
| Reaching for adjectives to sound premium | They make it sound generic. Replace with a number or mechanism. |
| Listing every feature "so they see the value" | Feature dumps bury the benefit. Climb to benefit, back with proof. |
| Dropping in any testimonial to fill the proof slot | A vague rave < one hard number. Quantify and attribute. |
| Five CTAs "to give them options" | Choice kills conversion. One primary action per viewport. |
| Hype words because "they test well" | They erode trust and trip the ban-list. Specificity converts. |
| Inventing a plausible metric to fill a gap | Never. Mark `[[NEEDS PROOF]]` and ask the user. |
| Reusing the same copy on X and LinkedIn | Resize ≠ adapt. Different format per platform, same claim. |
...
- [ ] One claim per asset — no post, email, or section carrying five hedged ones.
