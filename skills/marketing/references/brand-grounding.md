...
The brand study is the source of truth every piece of marketing copy is grounded in. This file holds the **completeness checklist** (what "complete" means), the **question script** (how to interview the user, batched), and the **persistence format** (how to write it into `docs` and link it from `CLAUDE.md`). The runtime mechanism that invokes this — the hard STOP on an incomplete study — lives in `SKILL.md` under "Brand grounding (read this first)".
...
Stored verbatim under `docs/raw/brand/` — see Raw links above.
```

### Raw samples

Paste each user-provided writing sample verbatim into its own `docs/raw/brand/voice-sample-N.md` with a one-line provenance header:
...
`docs/wiki/brand/`. Read it before writing any user-facing copy:
...
- [Proof](docs/wiki/brand/proof.md)
- [Offers & CTA](docs/wiki/brand/offers.md)
- [Channels](docs/wiki/brand/channels.md)
- [SEO keywords](docs/wiki/brand/seo.md)

Raw voice samples and inputs: `docs/raw/brand/`.
...
- `harness` — the canonical `docs` wiki protocol and article templates.
