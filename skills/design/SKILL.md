...
Follow the harness docs convention (brand study = wiki articles under `docs/wiki/brand/`, raw inputs under `docs/raw/brand/`, linked from root `CLAUDE.md`):

1. **Locate the brand study.** Read the project root `CLAUDE.md` and look for a `## Brand & voice` section pointing into `docs/wiki/brand/...`. If present, read those articles.
2. **If the link is MISSING, or the brand study is ABSENT or INCOMPLETE** (any checklist dimension empty), STOP. Do not design yet. Ask the user the targeted question script — **ONE focused batch at a time**, not a wall of questions — until every dimension in the completeness checklist is filled (→ `references/brand-grounding.md`). Write/update the brand study into `docs/wiki/brand/` (and paste any raw inputs the user gives — screenshots, existing palettes, competitor lists — into `docs/raw/brand/`), following the wiki article format, update `wiki/index.md` + `wiki/log.md`, and add/update a `## Brand & voice` section in the root `CLAUDE.md` linking to it (create `CLAUDE.md` if absent).
3. **Only once the brand study exists and is sufficient, proceed** — and cite which brand articles drove which decisions in your output (e.g. "palette from `docs/wiki/brand/visual-identity.md`").
...
| 11 | Copy & brand fidelity | Benefit-led, specific, ban-list clean, voice matches the `docs/wiki/brand/` study | 0.5 |
...
## Recording the decisions (docs)

Two records, both indexed in `docs/wiki/index.md` (the Knowledge map; root `CLAUDE.md` keeps only a short pointer to it), both read first on every use so outputs stay consistent with them:

- The **brand study** at `docs/wiki/brand/` — the hard gate above: missing or incomplete → ask until complete before designing.
