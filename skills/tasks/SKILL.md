...
in `docs/wiki/sdd/config.yaml`; when on, resolve the tier and apply it per
...
Read `docs/wiki/harness/user-profile.md` before producing anything and match
...
1. **The plan** — `docs/wiki/sdd/plans/<slug>.md`, written by the `plan`
   phase. It must contain the architecture, interfaces, and testing strategy.
2. **The spec** — `docs/wiki/sdd/specs/<slug>.md`, so every task traces to a
   requirement. A task with no spec line behind it is scope you are inventing.
3. **The constitution** — `docs/wiki/sdd/constitution.md`, the non-negotiable
   bars (stack canon, quality gates) every task must respect.
4. **The SDD config** — `docs/wiki/sdd/config.yaml`, if present. Use its
...
Then ensure the plan is indexed in `docs/wiki/index.md` (the Knowledge map;
...
`docs/wiki/sdd/plans/<slug>.md` and add it if missing. Additive only: never
